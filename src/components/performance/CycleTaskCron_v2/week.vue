<template lang="html">
  <div :val="value_">
    ...
    <div class="bottom" style="margin-top: 10px">
      ...
      <el-tag type="success">选择周几</el-tag>
      <el-checkbox-group v-model="appoint" style="margin-left: 0;  line-height: 25px;">
        <el-checkbox @change="type = '4'" v-for="(item,index) in weekData" :key="item.label" :label="item.label">{{item.key}}</el-checkbox>
      </el-checkbox-group>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      weekData:[{key:1,label:2},{key:2,label:3},{key:3,label:4},{key:4,label:5},{key:5,label:6},{key:6,label:7},{key:7,label:1}],
      type: '1', // 类型
      cycle: { // 周期
        start: 0,
        end: 0
