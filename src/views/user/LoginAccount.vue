  ...
  <div>
    <a-form-model ref="form" :model="model" :rules="validatorRules">
      <a-form-model-item required prop="username">
        <a-input v-model="model.username" size="large" placeholder="请输入帐户名或手机号">
...
// import { loginSupplier } from '@/api/login.js'
import Vue from 'vue'
...
export default {
  ...
  methods: {
    ...mapActions(['Login', 'LoginSupplier']),
    /**刷新验证码*/
    ...
    //账号密码登录
    handleLogin(rememberMe, isIdentity) {
      this.validateFields(['username', 'password', 'inputCode'], err => {
        if (!err) {
          ...
          // 这是供应商登录
            ...
            console.log(this.Login,this.LoginSupplier);
            this.LoginSupplier(loginParams)
              .then(res => {
                this.$emit('success', res.result)
              })
              .catch(err => {
