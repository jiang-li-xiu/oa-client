 ...
 * @LastEditors: JLX
 * @LastEditTime: 2022-12-26 14:14:38
-->
...
import detailDialog from './components/AddAndDetailDialog'
...
export default {
  ...
  data() {
    return {
      ...
      // 接口地址
      url: {
        list: 'prodQuoteApplyApprove/list',
      },
    }
  },
  created() {
  ...
  methods: {
    ...
    handleTabsChange(key) {
      ...
      if (key == 1) {
        ...
        this.url.list = `prodQuoteApplyApprove/list?pendingId=${pendingId}`
        this.loadData(1)
      } else if (key == 2) {
        // 已审批
        let approve = this.$store.getters.userInfo.id
        this.url.list = `prodQuoteApplyApprove/list?approve=${approve}`
        this.loadData(1)
      } else {
        // 全部
        this.url.list = `prodQuoteApplyApprove/list`
        this.loadData(1)
      }
    },
