  ...
  >
    <a-spin tip="加载中..." :spinning="loading">
      <a-form-model ref="ruleForm" :model="formParams" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-row>
          <a-col :span="24" class="title">方案名称：{{ formParams.name }}</a-col>
          <a-col :span="24">
            <template>
              <a-button
                class="buttom"
              ...
              <a-button
                ...
                >调整利润率</a-button
              >
              <a-button class="buttom" type="danger" @click="handleCleanProduct" v-if="[0, 1].includes(this.type)"
                >批量清除产品
              </a-button>
              <!-- <a-upload
              ...
              <template v-if="![2, 3, 4, 5].includes(this.type)">
                ...
                <div class="subtotal">
                  ...
                  <div class="number"></div>
                  <div class="number"></div>
                  <div class="number" :style="{ width: '200px' }"></div>
                  <!-- 采购合价 -->
              ...
              <template v-else>
                <a-table
                  ...
                  :data-source="dataSourceRemoveEmptyChild"
                  bordered
                  ref="tableformRef"
                  rowKey="key"
                  :scroll="{ x: true, y: 800 }"
                ...
                >
                  ...
                  <div class="overflowStyle" slot="salesTotlePrice" slot-scope="text, record">
                    <span style="white-space: pre-line">
                      {{ formatSalesPrice(record) }}
                      <!-- {{ record.num * record.price }} -->
                      <!-- {{ record.salesTotlePrice }} -->
                      <!-- {{ formatSalePrice }} -->
                    </span>
                  </div>
                  ...
                  <!-- <div class="overflowStyle" slot="action" slot-scope="text, record">
                    <span style="white-space: pre-line">
                      {{ record.remark }}
                    </span>
                  </div> -->
                ...
                <div class="subtotal">
                  ...
                  <div class="number">{{ formatSalePrice }}</div>
                  <div class="number"></div>
                  <div class="number" :style="{ width: '90px' }"></div>
                </div>
              </template>
            </template>
          </a-col>
    ...
    <div class="drawer-bootom-button" v-if="[2].includes(this.type)">
      ...
      <a-button @click="handleExportXls('利润表')" type="primary" class="sub-button">导出excel</a-button>
      <a-button @click="close">取消</a-button>
    </div>
...
export default {
  ...
  computed: {
    //去除数据的空child，避免出现扩展图标
    dataSourceRemoveEmptyChild() {
  ...
  watch: {
    ...
    dataSource: {
      handler(val) {
        if (val) {
          this.formatPurchasePrice = val.reduce((p, c) => p + Number(c.purchaseTotlePrice), 0).toFixed(2) //采购合价
          this.formatSalePrice = val.reduce((p, c) => p + Number(c.salesTotlePrice), 0).toFixed(2) //销售合价
          this.formatGrossProfitTotal = val.reduce((p, c) => p + Number(c.grossProfitRate), 0).toFixed(2) //毛利率
          let dataLength = val.length
          let purchaseTotlePrice = 0
          let salesTotlePrice = 0
          let grossProfitRate = 0
          // salesTotlePrice 销售核价
          let data = val.forEach((item) => {
            if (item.children) {
              ...
              grossProfitRate = item.children.reduce((p, c) => p + Number(c.grossProfitRate), 0).toFixed(2)
          ...
          this.$nextTick(() => {
            ...
            footerDom.addEventListener(
              "scroll",
              () => {
                console.log(this.$refs?.tableformRef?.$el.getElementsByClassName("ant-table-body")[0].scrollLeft)
                // console.log(footerDom.scrollLeft)
  ...
  methods: {
    ...
    handleExportXls(fileName) {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          let url = "/prodProfits/download/excel/"
          downFile(url + `${this.approvalId}`).then((data) => {
            if (!data) {
    ...
    edit(record) {
      this.type = 1
      this.title = "编辑利润表"
      this.$set(this.formParams, "name", record.name)
    ...
    // 详情
    detail(record, projectInfo) {
      console.log("详情", record, projectInfo)
      //项目信息
    ...
    changeNum(row, i) {
      ...
      let converProfit = profit * 100
      // 销售单价：price  采购单价：purchasePrice
      // console.log('毛利润----', profit,profits, '销售合价--', dataSalesPrice, '采购合价--', data)
      ...
      if (row.salesTotlePrice !== "0.00") {
        ...
        // 改毛利率
        is_children
          ? this.$set(this.dataSource[i], "grossProfitRate", Number(converProfit).toFixed(2))
          : this.$set(dataFristLeval[0].children[i], "grossProfitRate", Number(converProfit).toFixed(2))
      } else {
        // console.log('等于0')
        // 改毛利率
        is_children
    ...
    changeGrossProfitRate(changeVal, row, i) {
      ...
      //改毛利率
      is_children
        ? this.$set(this.dataSource[i], "grossProfitRate", Number(changeVal).toFixed(2))
        : this.$set(dataFristLeval[0].children[i], "grossProfitRate", Number(changeVal).toFixed(2))
      // console.log('当前的毛利润', row.grossProfitRate)

      // 计算销售合价和单价  purchasePrice:采购单价  销售单价：price = 成本/(1-毛利)
      const grossProfitRate = Number(row.grossProfitRate / 100)
      console.log("转换后的毛利率", grossProfitRate)
      let salesPrice = row.purchasePrice / (1 - grossProfitRate)
      // 销售合价
    ...
    changeProfit(data) {
      ...
      let converProfit = Number(data.profit / 100)
      // console.log(converProfit);

      // 最终的总价和单价; converProfit:当前毛利润
      this.selectionRows.map((item, index) => {
        console.log(item, index)
        item.price = item.purchasePrice / (1 - converProfit) //销售单价=采购单价/（1-毛利润）
    ...
    addCurrentRow(row, index) {
      ...
      this.dataSource[index].updateStatus = "1"
      // 先生成一个随机key
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          for (let keys in this.dataSource) {
            ...
            for (let key in list) {
              if (
                ...
                key !== "childId" &&
                key !== "updateStatus" &&
                [null, undefined, ""].includes(list["price"])
                // [null, undefined, ''].includes(list['grossProfitRate'])&&
                // [null, undefined, ''].includes(list['salesTotlePrice'])
              ) {
                if (!list[key]) {
                  return this.$message.warning("请完整填写带*的产品信息!")
                }
              ...
              for (let keys in list.children) {
                ...
                for (let key in listChildren) {
                  if (
                    ...
                    key !== "salesPrice" &&
                    key !== "totalAmount" &&
                    [null, undefined, ""].includes(listChildren["price"])
                  ) {
                    // console.log(listChildren)
                    if (!listChildren[key]) {
          ...
          let data = {
            ...
            // profitsProducts: this.dataSource,
            // project: project,
            schemeLibraryId: this.currentData.id,
            profitsProducts: this.dataSource,
          }
          console.log(data)
          editProfit(data)
