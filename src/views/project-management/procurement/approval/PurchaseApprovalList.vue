          ...
          <a-col :md="6" :sm="8">
            <a-form-model-item label="申请类别" prop="type">
              ...
              <a-select v-model="param.type" placeholder="请选择申请类别">
                <a-select-option value="4">部门采购申请</a-select-option>
                <a-select-option value="6">三方比价申请</a-select-option>
                <a-select-option value="7">采购下单申请</a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
          <a-col :md="6" :sm="8">
...
export default {
  ...
  components: { PurchaseRequestForm },
  props: {},
  data() {
    return {
      // 查询表单
      ...
      // 接口地址
      url: {
        list: '/prodApprove/list/purchase',
      },
    }
  },
  created() {
  ...
  methods: {
    ...
    handleTabsChange(key) {
      ...
      if (key == 1) {
        // 待审批
        this.url.list = `/prodApprove/list/purchase?isApproved=1&approverId=*${this.$store.getters.userInfo.id}*`
        this.loadData(1)
      } else if (key == 2) {
        // 已审批
        this.url.list = `/prodApprove/list/purchase?isApproved=2&approverId=*${this.$store.getters.userInfo.id}*`
        this.loadData(1)
      } else {
        // 全部
        this.url.list = `/prodApprove/list/purchase`
        this.loadData(1)
      }
    },
    // 处理搜索时间
    ...
    getParam() {
      let param = {
        // type: `*${this.queryParam.type || null}*`,
        type: this.queryParam.type,
        applyTime_begin: this.queryParam.applyTime_begin,
        applyTime_end: this.queryParam.applyTime_end,
        applyPeople: this.queryParam.applyPeople,
        applyTime: this.queryParam.applyTime,
