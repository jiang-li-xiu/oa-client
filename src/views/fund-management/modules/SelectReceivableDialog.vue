          ...
          <a-form-model ref="ruleForm" :model="param" layout="inline" @keyup.enter.native="searchQuery">
            <slot :param="param">
              <a-form-model-item label="编号" prop="num">
                <j-input v-model="param.num" placeholder="请输入编号"></j-input>
              </a-form-model-item>
            </slot>
            <a-form-model-item>
...
export default {
  ...
  props: {
    ...
    listUrl: {
      require: true,
      type: String,
      default: '/prodInvoice/invoice/list',
    },
    // 单选或多选
    selectType: {
      type: String,
  ...
  methods: {
    ...
    loadData(arg) {
      ...
      this.loading = true
      // 处理url
      // this.url.list = this.type == 1 ? '/prodReceivable/return/list' : '/prodReturnedMoney/list'

      // this.url.list = this.is_type
      //   ? `${this.url.list}?contractId=${this.contractId}&customerId=${this.customerId}&type=${this.type}`
      //   : `${this.url.list}?contractId=${this.contractId}&customerId=${this.customerId}`
