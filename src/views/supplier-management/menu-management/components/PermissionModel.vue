...
<script>
import { addSupplierPermission, editSupplierPermission, querySupplierTreeList, duplicateCheck } from '@/api/api'
import Icons from '@/views/system/modules/icon/Icons.vue'
...
export default {
  ...
  methods: {
    loadTree() {
      var that = this
      querySupplierTreeList().then((res) => {
        if (res.success) {
          console.log(res)
          that.treeData = []
    ...
    handleOk() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          let obj
          if (!this.model.id) {
            obj = addSupplierPermission(this.model)
          } else {
            obj = editSupplierPermission(this.model)
          }
          obj
            .then((res) => {
              if (res.success) {
