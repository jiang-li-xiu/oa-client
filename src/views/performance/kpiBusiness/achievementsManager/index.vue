  ...
  <div class="app-container">
    ...
    <div class="bg-border-color-10">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-tag :type="getStyle(scope.row)">
              {{ scope.row.kpiCheckNapeVoList[0].assessmentStatusLabel }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <router-link
              :to="
                '/assessment/manager/data/' + scope.row.assessmentConfigureId
              "
              class="link-type"
            >
              <span>考核项详情/跳评</span>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAssessmentConfigureAdmin(this.queryParams).then((response) => {
    ...
    urgeFill() {
      this.urgeFillOpen = true;
      this.title = "催填";
    },
    //获取考核项样式
    getStyle(row) {
      return this.StatusType[
        parseInt(row.kpiCheckNapeVoList[0].assessmentStatus)
      ];
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          urgeFillAssessment(this.form).then((response) => {
            this.msgSuccess("催填成功！！");
            this.urgeFillOpen = false;
          });
        }
      });
