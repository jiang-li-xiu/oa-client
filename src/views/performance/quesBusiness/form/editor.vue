...
import render from '@/components/performance/render/render'
import RightPanel from './RightPanel'

import {
    inputComponents, selectComponents, formConf
} from '@/components/performance/generator/config'
import {
    exportDefault, beautifierConf, isNumberStr, titleCase, deepClone, jsonClone
} from '@/utils/index'
import {dbDataConvertForItemJson, formItemConvertData} from '@/utils/performance/convert'
import drawingDefalut from '@/components/performance/generator/drawingDefalut'
// import DraggableItem from './DraggableItem'
import {
    getDrawingList, saveDrawingList, getIdGlobal, saveIdGlobal, getFormConf
} from '@/utils/performance/db'
...
import { 
    queryProjectByKey,
    queryProjectMaxFormItemId,
    saveProjectInfo,
    deleteProjectItemInfo,
    queryProjectItems,
    onItemEnd,
    saveProjectItemInfo,
} from "@/api/performance/tduck";
...
export default {
    ...
    mounted() {
        // 复制对象 避免修改改变原始对象
        console.log("edit",this.projectKey);
        ...
        // })
        queryProjectByKey(projectKey).then(res=>{
            this.formConf.title = res.data.name
            this.formConf.description = res.data.describe
        })
        //全局组件Id
        ...
        // })
        queryProjectMaxFormItemId({key: this.projectKey}).then(res=>{
            this.idGlobal = res.data ? res.data : 100
        })
        Cookies.set('quesID',projectKey)
    ...
    methods: {
        ...
        deleteProjectItemInfo(val) {
            ...
            // })
            deleteProjectItemInfo(data).then(res=>{})
        },
        saveProjectItemInfo(item) {
            ...
            // })
            saveProjectItemInfo(params).then(res=>{
                item.sort = res.data.sort
            })
        },
        queryProjectItems() {
            ...
            // })
            queryProjectItems({params: {key: this.projectKey}}).then(res=>{
                this.drawingList = res.data.map(item => dbDataConvertForItemJson(item))
            })
        },
...
<style lang='scss'>
@import '@/views/performance/assets/styles/form/home';
@import '@/views/performance/assets/styles/form/index';
</style>
