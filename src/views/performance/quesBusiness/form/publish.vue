...
import { queryProjectByKey,publishProject,stopPublishProject } from "@/api/performance/tduck";
export default {
    name: 'projectPublish',
    components: {
        VueQr
    ...
    data() {
        ...
        getProjectStatus() {
            ...
            //     }
            // })
            queryProjectByKey(this.projectKey).then(res=>{
                if (res.data.status == 2) {
                    this.publishStatus = true
                }else{
                    this.publishStatus = false
        ...
        publishProject() {
            ...
            // })
            publishProject({key: this.projectKey}).then(res=>{
                this.publishStatus = true
                this.msgSuccess('发布成功')
            })
        },
        stopPublishProject() {
            ...
            //     }
            // })
            stopPublishProject({'key': this.projectKey}).then(res=>{
                if (res.data) {
                    this.msgSuccess('停止成功')
                    this.getProjectStatus()
                } 
