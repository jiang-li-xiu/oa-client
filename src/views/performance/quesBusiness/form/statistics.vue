            ...
            <el-drawer
                ...
                <el-scrollbar style="height:100%">
                    <el-card class="box-card">
                        ...
                        <div>
                            <div v-for="(item,index) in projectItemList" :key="index">
                                <h4>{{ item.label }}</h4>
                                <!--  如果是文件输入-->
                                <div v-if="item.type==17 &&activeResultRow&&activeResultRow['processData'][`field${item.formItemId}`]">
                                    <el-link
                                        ...
                                        v-for="(file,inx) in JSON.parse(activeResultRow['processData'][`field${item.formItemId}`]['files'])" :key="inx">
                                        {{ file.fileName }}
                                    </el-link>
                                </div>
                                <el-tag v-else> {{
        ...
        <div class="custom-col-container">
            <el-dialog center title="自定义显示列" :visible.sync="customColumnDialogVisible">
                ...
                <el-checkbox-group v-model="checkFixedCustomColumns">
                    <el-row>
                        <el-col :span="4" v-for="(val, key)  in fixedDefaultLabelFormColumn" :key="key">
                ...
                <el-checkbox-group v-model="checkOtherCustomColumns">
                    <el-row>
                        <el-col :span="8" v-for="(val, key) in projectItemColumns" :key="key">
                            <el-checkbox :label="key">{{ val }}</el-checkbox>
                        </el-col>
                    </el-row>
                </el-checkbox-group>
...
import {getCheckedColumn, saveCheckedColumn} from '@/utils/performance/db'
import {queryProject,queryProjectResult,exportProjectResult,queryProjectItems} from "@/api/performance/tduck"
...
export default {
    ...
    data() {
        ...
        queryProjectResult() {
            // this.$api.get(`/user/project/user/project/result/page`, {params: this.queryConditions}).then(res => {
            //     let {records, total, size} = res.data
            //     this.projectResultList = records
            //     this.total = total
            //     this.queryConditions.size = size
            // })
            queryProjectResult({params: this.queryConditions}).then(res=>{
                let {records, total, size} = res.data
                this.projectResultList = records
                this.total = total
                this.queryConditions.size = size 
