          ...
          <table :border="1">
            ...
            <tbody>
              <tr>
                ...
                <td>
                  <a-form-model-item class="flex-row" label="申请人" prop="userId">
                    <a-input v-model="model.userName" placeholder="请输入申请人" :read-only="true"></a-input>
                  </a-form-model-item>
                </td>
                <td>
                      ...
                      <a-row type="flex" style="margin-bottom:3px" :gutter="4" v-for="(item, index) in model.feeDetails" :key="index">
                        ...
                        <a-col :span="5">
                          <a-form-model-item :prop="'feeDetails.' + index + '.amount'" :rules="[{required: true,message: '请输入金额',trigger: 'blur'}]">
                            <a-input-number placeholder="金额" @blur="rowChange" :min="0.01" :max="999999999.99" v-model="item.amount" style="width: 100%;"/>
                          </a-form-model-item>
                        </a-col>
                        <a-col :span="4">
                          <a-form-model-item>
              ...
              <tr>
                <td colspan="3" style="padding:6px 12px 12px 12px;">
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          create: "/scrm/flwExpenseReimbursement/create",
        },
        total:0,
        selected:true
      }
      ...
      handleOks () {
        ...
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            //构建variables
            that.initializeVar();
            httpAction(this.url.create, this.model, 'post').then((res)=>{
              if(res.success){
