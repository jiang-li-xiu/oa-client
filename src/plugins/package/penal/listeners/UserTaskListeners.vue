...
export default {
  ...
  methods: {
    resetListenersList() {
      ...
      this.otherExtensionList = this.bpmnElement.businessObject?.extensionElements?.values?.filter(ex => ex.$type !== `${this.prefix}:TaskListener` || (ex.$type === `${this.prefix}:TaskListener` && !isEmpty(ex.eventType))) ?? [];
      this.bpmnElementListeners = this.bpmnElement.businessObject?.extensionElements?.values?.filter(ex => ex.$type === `${this.prefix}:TaskListener` && isEmpty(ex.eventType)) ?? [];
    ...
    openListenerForm(listener, index) {
      if (listener) {
        ...
        this.listenerForm = { id: `Listener_${uuid(8)}` };
    ...
    async saveListenerConfig() {
      ...
      if (this.editingListenerIndex === -1) { // -1为新增标记
