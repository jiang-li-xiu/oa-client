<template>
  <div class="process-design" :style="'display: flex; height:' + height">
    <bpmn-process-designer
      ...
      @save="onSaveProcess"
    />
    <bmpn-process-penal :bpmn-modeler="modeler" :conditions="conditions" :condition-list="conditionList" :prefix="controlForm.prefix" @ok="handleConditionsList" class="process-panel" />
  </div>
</template>
...
export default {
  ...
  methods: {
    design(record){
      this.controlForm.bpmnXml = record.processXml;
      this.xmlString = record.processXml;
      this.controlForm.processName = record.processName;
      this.controlForm.processId = record.id;
    },
    ...
    /**从properties panel回传流程节点条件 */
    handleConditionsList(data){
      this.conditionList = data
    },
    onSaveProcess (saveData) {
      this.$emit('save', saveData, this.conditionList);
    }
  }
}
</script>
