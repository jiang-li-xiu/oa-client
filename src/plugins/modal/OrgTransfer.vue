                  ...
                  <template v-for="elem in roleList">
                    ...
                    >
                      <a :title="item.description" :class="{ active: elem.isActiveItem && elem.isActiveItem(item) }">
                        <img src="@/assets/icon_role.png" />{{ item.name }}
                      </a>
                    </li>
                  </template>
                </ul>
      ...
      get() {
        ...
        if(this.checkedRoleList && this.checkedRoleList.length > 0){
          data.push({
            ...
            cancel: (item) => this.removeEle(this.checkedRoleList, item),
          })
        }
        if(this.checkedEmployessList && this.checkedEmployessList.length > 0){
          data.push({
            type: 'employee',
        ...
        if(this.checkedDepartmentList && this.checkedDepartmentList.length > 0){
          data.push({
            type: 'department',
  ...
  methods: {
    ...
    getCheckedData() {
      this.checkedDatas.map((item) => {
        // 判断类型
        if (item.type === 'role' && item.data.length > 0) {
          this.checkedRoleList = item.data // 已选角色数据
        } else if (item.type === 'employee' && item.data.length > 0) {
          this.checkedEmployessList = item.data // 已选人员数据
        } else if (item.type === 'department' && item.data.length > 0) {
          ...
          this.roleList = [
            {
              type: 'role',
              data: res.result,
              isActiveItem: (item) => this.toggleClass(this.checkedRoleList, item),
              change: (item) => this.toChecked(this.checkedRoleList, item),
            },
          ]
        } else {
          that.$message.warning(res.message)
    ...
    confirm() {
      const res = {}
      let data = this.checkedData.map((item, i) => {
