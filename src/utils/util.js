 ...
 */
export function throttle(func, wait = 5000, type = 1) {
    let previous = 0;
    let timeout;
    return function() {
        let context = this;
 ...
 * @return {string}
 */
export function byteChange(byte) {
    let size = 0
    if (byte == null || byte === 'null' || byte === 'undefined') {
        size = 0
    } else if (byte < 0.1 * 1024) {
        // 小于0.1kb，则转为B
    ...
    var d = sizeStr.substring(index + 1, 2)
    if (d == '00') {
